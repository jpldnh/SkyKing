 ...
 * @author YANG ZT
 */
public class SKRListViewExpand extends ListView {

	private SparseArray<SparseArray<SKImageTask>> tasks = new SparseArray<SparseArray<SKImageTask>>();
	private SKImageWorker imageWorker;// 图片下载器
	private OnScrollListener onScrollListener;// 由于设置了自定义滑动监听,如果再设置滑动监听的话只需记录一下,在自定义监听中调用即可

	public SKRListViewExpand(Context context) {
		this(context, null);
	}

	public SKRListViewExpand(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public SKRListViewExpand(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(Context context) {
		...
		imageWorker = new SKImageWorker(context.getApplicationContext());
	 ...
	 *            the task
	 */
	public void addTask(int position, int index, SKImageTask task) {
		...
		// 需要异步执行的任务，添加进任务队列
		SparseArray<SKImageTask> tasksInPosition = tasks.get(position);
		if (imageWorker.loadImage(task)) {
			if (tasksInPosition == null) {
				tasksInPosition = new SparseArray<SKImageTask>();
	...
	private void excuteTasks(int first, int last) {
		for (int i = first; i <= last; i++) {
			SparseArray<SKImageTask> tasksInPosition = tasks.get(i);
			if (tasksInPosition != null) {
				int size = tasksInPosition.size();
				for (int index = 0; index < size; index++) {
					int key = tasksInPosition.keyAt(index);
					SKImageTask task = tasksInPosition.get(key);
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			case SCROLL_STATE_IDLE:
				...
				if (xtomScrollListener != null)
					xtomScrollListener.onStop(SKRListViewExpand.this);
				break;
			case SCROLL_STATE_TOUCH_SCROLL:
				imageWorker.clearTasks();
				imageWorker.setThreadControlable(true);
				if (xtomScrollListener != null)
					xtomScrollListener.onStart(SKRListViewExpand.this);
				break;
			default:
				break;
			}
	...
	public interface XtomScrollListener {
		 ...
		 * @param view
		 */
		public void onStart(SKRListViewExpand view);
		 ...
		 * @param view
		 */
		public void onStop(SKRListViewExpand view);
	}
	...
	public interface XtomSizeChangedListener {
		public void onSizeChanged(SKRListViewExpand view, int w, int h, int oldw,
                                  int oldh);
	}
}
