 ...
 */
@SuppressLint("NewApi")
public class SKRWheelView extends View {
	...
	SKRInterfaces.SKROnItemSelectedListener onItemSelectedListener;
	...
	SKRWheelAdapter<Object> adapter;
	...
	private static final float SCALECONTENT = 0.9F;// 非中间文字则用此控制高度，压扁形成3d错觉，数值月下间隔越大
	private static final float CENTERCONTENTOFFSET = 4;// 中间文字文字居中偏移值
	private static final float FIRSTLINECONTENTOFFSET = 3;// 第二条横线偏移值,正代表上移，负向下
	private static final float SECONLINEDCONTENTOFFSET = -2;// 第二条横线偏移值，正代表上移，负向下
	private static final String GETPICKERVIEWTEXT = "getPickerViewText";// 反射的方法名

	public SKRWheelView(Context context) {
		this(context, null);
	}

	@SuppressLint("Recycle")
	public SKRWheelView(Context context, AttributeSet attrs) {
		super(context, attrs);
		textColorOut = getResources().getColor(R.color.color_text_gray_g);
		textColorCenter = getResources().getColor(R.color.color_text_gray_g);
		dividerColor = getResources().getColor(R.color.color_line_gray_d);
		textSizeCenter = SKTransfromUtil.dipTopx(context,18);
		textSizeOut = SKTransfromUtil.dipTopx(context,14);

		if (attrs != null) {
			TypedArray a = context.obtainStyledAttributes(attrs,
					R.styleable.SKR_AttrsWheelView, 0, 0);
			mGravity = a.getInt(R.styleable.SKR_AttrsWheelView_gravity, Gravity.CENTER);
			textColorOut = a.getColor(R.styleable.SKR_AttrsWheelView_textColorOut,
					textColorOut);
			textColorCenter = a.getColor(R.styleable.SKR_AttrsWheelView_textColorCenter,
					textColorCenter);
			dividerColor = a.getColor(R.styleable.SKR_AttrsWheelView_dividerColor,
					dividerColor);
			textSizeOut = a.getDimension(R.styleable.SKR_AttrsWheelView_textSizeOut,
					textSizeOut);
			textSizeCenter = a.getDimension(
					R.styleable.SKR_AttrsWheelView_textSizeCenter, textSizeCenter);
			itemsVisible = a.getInt(
					R.styleable.SKR_AttrsWheelView_itemcount, itemsVisible);
		}
	...
	private void remeasure() {
		...
		// 计算两条横线和控件中间点的Y位置
		firstLineY = (measuredHeight - itemHeight) / 2.0F - FIRSTLINECONTENTOFFSET;
		secondLineY = (measuredHeight + itemHeight) / 2.0F - SECONLINEDCONTENTOFFSET;
	...
	public final void setAdapter(SKRWheelAdapter adapter) {
		this.adapter = adapter;
		remeasure();
		invalidate();
	}

	public final SKRWheelAdapter getAdapter() {
		return adapter;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		while (counter < itemsVisible) {
			...
			if (angle >= 90F || angle <= -90F) {
				...
				canvas.scale(1.0F, (float) Math.sin(radian));//注掉后最上边最下边感觉现实更好点
	...
	private class MessageHandler extends Handler {
		...
		final SKRWheelView loopview;

		MessageHandler(SKRWheelView loopview) {
		...
		public final void handleMessage(Message msg) {
			switch (msg.what) {
				...
				case WHAT_SMOOTH_SCROLL:
					loopview.smoothScroll(SKRWheelView.ACTION.FLING);
					break;
	...
	private class LoopViewGestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		private SKRWheelView loopView;

		LoopViewGestureListener(SKRWheelView loopview) {
			loopView = loopview;
		}
	...
	private class OnItemSelectedRunnable implements Runnable {
		...
		OnItemSelectedRunnable(SKRWheelView loopview) {
			loopView = loopview;
		}
	...
	private class InertiaTimerTask extends TimerTask {
		...
		InertiaTimerTask(SKRWheelView loopview, float velocityY)
		{
			super();
			loopView = loopview;
	...
	private class SmoothScrollTimerTask extends TimerTask {
		...
		SmoothScrollTimerTask(SKRWheelView loopview, int offset) {
			this.loopView = loopview;
			this.offset = offset;
