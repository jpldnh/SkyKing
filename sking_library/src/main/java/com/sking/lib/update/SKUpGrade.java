 ...
 * 软件升级
 */
public class SKUpGrade extends SKObject {
	 ...
	 */
	public SKUpGrade(Context mContext, String appName, String url) {
	...
	void showerr() {
		pBar.cancel();
		SKToastUtil.showShortToast(mContext, "下载出错");
	}
	...
	void showcancel() {
		...
		if (file.exists())
			file.delete();
		SKToastUtil.showShortToast(mContext, "更新已取消");
	}
		...
		new Thread() {
			public void run() {
				...
				try {
					...
					if (is != null) {
						saveappdir = SKFileUtil.getTempFileDir(mContext);
