...
import com.sking.lib.db.SKDBHelper;

import java.util.ArrayList;

class SKFileLoadDBHelper extends SKDBHelper {
	...
	private static SKFileLoadDBHelper dbHelper;

	private SKFileLoadDBHelper(Context context) {
		super(context);
	}

	static synchronized SKFileLoadDBHelper getInstance(Context context) {
		return dbHelper == null ? dbHelper = new SKFileLoadDBHelper(context)
				: dbHelper;
	}

	public boolean insertOrUpdateThreadInfo(SKThreadInfo info) {
		synchronized (this) {
			if (!isExist(info))
	...
	public boolean isExist(SKThreadInfo info) {
		synchronized (this) {
	...
	public boolean insertThreadInfo(SKThreadInfo info) {
		synchronized (this) {
	...
	public boolean updateThreadInfo(SKThreadInfo info) {
		synchronized (this) {
			boolean success = true;
	...
	public boolean insertOrUpdateFileInfo(SKFileInfo info) {
		synchronized (this) {
			if (!isExist(info))
	...
	public boolean insertFileInfo(SKFileInfo info) {
		synchronized (this) {
			boolean success = true;
	...
	public boolean updateFileInfo(SKFileInfo info) {
		synchronized (this) {
			boolean success = true;
	...
	public boolean isExist(SKFileInfo info) {
		synchronized (this) {
			String conditions = "downPath='" + info.getDownPath()
					+ "' and savePath='" + info.getSavePath()
					+ "' and threadCount=" + info.getThreadCount();
	...
	public SKFileInfo getFileInfo(String downPath, String savePath,
								  int threadCount) {
		synchronized (this) {
			...
			String sql = "select " + fileColumns + " from " + FILEDOWNLOAD
					+ " where " + conditions;
			SKFileInfo info = null;
			Cursor cursor = getReadableDatabase().rawQuery(sql, null);
			if (cursor != null && cursor.getCount() > 0) {
				cursor.moveToFirst();
				info = new SKFileInfo(cursor.getInt(0), cursor.getString(1),
	...
	public SKThreadInfo getThreadInfo(int fileID, int threadID) {
		synchronized (this) {
			String conditions = "fileID=" + fileID + " and threadID="
					+ threadID;
			String sql = "select " + threadColumns + " from "
					+ FILEDOWNLOADTHREAD + " where " + conditions;
			SKThreadInfo info = null;
			Cursor cursor = getReadableDatabase().rawQuery(sql, null);
			if (cursor != null && cursor.getCount() > 0) {
				cursor.moveToFirst();
				info = new SKThreadInfo(cursor.getInt(0), cursor.getInt(1),
	...
	public ArrayList<SKThreadInfo> getThreadInfos(int fileID) {
		synchronized (this) {
			...
			Cursor cursor = getReadableDatabase().rawQuery(sql, null);
			ArrayList<SKThreadInfo> infos = null;
			if (cursor != null && cursor.getCount() > 0) {
				infos = new ArrayList<SKThreadInfo>();
				cursor.moveToFirst();
				SKThreadInfo info;
