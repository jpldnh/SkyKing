 ...
 * 文件下载器(多线程异步断点续传)
 */
public class SKFileDownLoader extends SKObject {
	...
	private SKFileInfo fileInfo;// 下载文件信息

	private ControlThread controlThread;

	private SKFileDownLoader() {
		Looper looper;
	 ...
	 *            文件保存地址
	 */
	public SKFileDownLoader(Context context, String downPath, String savePath) {
		this();
	...
	public boolean isFileLoaded() {
		...
		SKFileInfo fileInfo = getFileInfo();
		if (isLoaded) {
			// 如果文件存在,再判断下载进度
			if (fileInfo != null) {
			...
			if (fileInfo != null) {
				...
				SKFileLoadDBHelper dbHelper = SKFileLoadDBHelper
						.getInstance(context);
	...
	private class ControlThread extends Thread {
		private SKFileLoadDBHelper dbHelper;// 数据库帮助工具
		private ArrayList<SKFileLoadThread> downLoadThreads = new ArrayList<SKFileLoadThread>();

		// 停止下载
		private void stopLoad() {
			for (SKFileLoadThread thread : downLoadThreads) {
				thread.setStop(true);
			}
		}

		// 获取文件信息
		private SKFileInfo getFileInfo(int contentLenght) {
			// 先从数据库取
			SKFileInfo fileInfo = dbHelper.getFileInfo(downPath, savePath,
					threadCount);

			if (fileInfo == null) {
				// 如果数据库没有,新建一个
				fileInfo = new SKFileInfo(0, downPath, savePath, threadCount,
						contentLenght, 0);
				// 插入数据库
		...
		// 获取下载线程信息
		private SKThreadInfo getThreadInfo(int fileID, int threadID,
										   int startPosition, int endPosition) {
			// 先从数据库取
			SKThreadInfo info = dbHelper.getThreadInfo(fileID, threadID);

			if (info == null) {
				// 如果数据库没有,新建一个
				info = new SKThreadInfo(0, fileID, threadID, startPosition,
			...
			if (isFileLoaded()) {
				...
				try {
					...
					for (int i = 0; i < threadCount; i++) {
						...
						SKThreadInfo info = getThreadInfo(fileInfo.getId(), i,
								startPosition, endPosition);

						SKFileLoadThread fdt = new SKFileLoadThread(context, url,
								file, info);
		...
		private void checkThreads() {
			...
			for (int i = 0; i < downLoadThreads.size(); i++) {
				SKFileLoadThread thread = downLoadThreads.get(i);
	...
	private static class EventHandler extends android.os.Handler {
		...
		public EventHandler(SKFileDownLoader downLoader, Looper looper) {
			super(looper);
		...
		public void handleMessage(Message msg) {
			SKDownLoadListener listener = downLoader
	 ...
	 */
	public SKFileInfo getFileInfo() {
		if (fileInfo == null) {
			// DownLoadDBHelper dbHelper = new DownLoadDBHelper(context);
			SKFileLoadDBHelper dbHelper = SKFileLoadDBHelper.getInstance(context);
	...
	private SKDownLoadListener xtomDownLoadListener;

	public SKDownLoadListener getXtomDownLoadListener() {
		return xtomDownLoadListener;
	}

	public void setXtomDownLoadListener(
			SKDownLoadListener xtomDownLoadListener) {
