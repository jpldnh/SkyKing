 ...
 */
public class SKMedia extends SKObject {
	 ...
	 * @return
	 */
	public static SKMedia get(Context context) {
		return new SKMedia(context);
	}

	private SKMedia(Context context) {
	 ...
	 *            语音文件类型(1本地文件2网络文件)
	 */
	public SKMedia(Context context, String path, int type) {
		this(context);
		if (isNull(path))
	...
	public void start() {
		if (type == 2) {
			if (isLoading) {
				SKToastUtil.showShortToast(context, "正在等待播放,请稍后。");
				return;
			}
		...
		try {
			...
			SKToastUtil.showShortToast(context, "文件损坏,播放失败.");
			log_i("播放失败");
			if (type == 2) {
	...
	private void load() {
		new AsyncTask<Object, Object, Object>() {
			...
			protected void onPostExecute(Object result) {
				...
				if (isSuccess) {
					SKToastUtil.showShortToast(context, "开始播放");
					if (isPlayableAfterDownload)
						start();
				} else {
					SKToastUtil.showShortToast(context, "播放失败");
				}
