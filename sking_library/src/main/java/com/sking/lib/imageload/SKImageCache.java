 ...
 * 图片缓存
 */
public class SKImageCache extends SKObject {
	private Context mContext;
	private static SKImageCache imageCache;
	...
	private SKImageCacheInMemByObject cacheInMemByObject;// 图片在不同场合中的使用情况
	 ...
	 * @return
	 */
	public static synchronized SKImageCache getInstance(Context context) {
		return (imageCache == null) ? imageCache = new SKImageCache(
	 ...
	 * @return
	 */
	public static synchronized SKImageCache get() {
		return imageCache;
	}

	private SKImageCache(Context context) {
		mContext = context;
		...
		cacheInMemByObject = new SKImageCacheInMemByObject();// 实例化图片在不同Activity中的使用情况
		deleteCache(cachePath_external, SKConfig.IMAGES_EXTERNAL);// 若外部缓存中那个图片数量超过限制,删除一半的缓存
		deleteCache(cachePath_internal, SKConfig.IMAGES_INTERNAL);// 若内部缓存中那个图片数量超过限制,删除一半的缓存
	}
	...
	private void cacheInMemByObj(SKImageTask task, Bitmap bitmap) {
	...
	public String getPathAtLoacal(String path) {
		...
		if (file.exists())
			return path;
		String temp = SKFileUtil.getKeyForCache(path);
		return SKFileUtil.isExternalMemoryAvailable() ? cachePath_external
	 ...
	 * @return
	 */
	public Bitmap getBitmapFromLocal(SKImageTask task) {
		URL url = task.getUrl();
		Bitmap bitmap = null;
		if (url != null) {// =================网络图片,执行以下操作=================
			...
			if (bitmap == null) // 若内存中没有,在本地缓存(外部缓存)中取
			{
				String lpath = cachePath_external
						+ SKFileUtil.getKeyForCache(path);
	 ...
	 * @return
	 */
	public Bitmap getBitmapFromServer(SKImageTask task) {
	...
	// 获取本地图片
	private Bitmap getLocalPicture(String path, SKSize size) {
		Bitmap bitmap;
		try {
			if (size == null)
				bitmap = SKImageUtil.getLocalPicture(path);
			else
				bitmap = SKImageUtil.getLocPicByDBYS(path, size.getHeight(),
						size.getWidth());
			return bitmap;
		} catch (IOException e) {
			bitmap = null;
	...
	// 从服务器获取图片
	private Bitmap load(URL url, String path, SKImageTask task) {
		if (!isCanLoad()) {
			return null;
		}
		...
		try {
			...
			conn.setConnectTimeout(SKConfig.TIMEOUT_CONNECT_IMAGE);
			conn.setReadTimeout(SKConfig.TIMEOUT_READ_IMAGE);
			...
			if (!dir.exists())
				dir.mkdir();
			file = new File(strdir + SKFileUtil.getKeyForCache(path));
	 ...
	 * @return
	 */
	public Bitmap getFromMemCache(SKImageTask task) {
	...
	private boolean isCanLoad() {
		if (!SKConfig.IMAGELOAD_ONLYWIFI) {
			return true;
		}
		boolean isWifi = true;
		try {
